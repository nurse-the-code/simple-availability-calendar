...
| `src/calendar.js`         | No         | Merge dates + status, render grid, attach tooltips |
...
| `src/calendar.js`             | Regular script (global) | Rendering logic; reads globals for browser use                    |
...
3. **Separation of generated vs user data**: `calendar-dates.js` can be regenerated without losing user edits in `calendar-status.js`.

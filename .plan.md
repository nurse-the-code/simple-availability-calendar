...
A static availability calendar built with 5 files (HTML, CSS, 2 JS data files, JS logic) that runs locally via `open calendar.html`. No server required.
...
2. **Edit** — Manually fill in `status` and `notes` in `calendar-status.js`
...
| **2** | Generator tool creates `calendar-dates.js` (padded weeks + Hebrew dates) |        |
| **3** | JS renders grid with dates from `calendar-dates.js`                      |        |
| **4** | Status colors applied from `calendar-status.js`                          |        |
| **5** | Saturdays automatically marked unavailable                               |        |
| **6** | Dynamic legend (only shows statuses present in data)                     |        |
| **7** | Hover tooltips                                                           |        |
...
### calendar-status.js (User-Edited)
...
| `calendar-status.js` | No (user)  | Title, availability status and notes per date      |
| `calendar.js`        | No         | Merge dates + status, render grid, attach tooltips |
...
| `calendar-status.js` | Regular script (global) | Sets `CALENDAR_STATUS` global for browser use                  |
| `calendar.js`        | ES module               | Exports pure utility functions; reads globals for browser use  |
| `*.test.js`          | ES module               | Imports functions from calendar.js, injects test data          |

### Why This Structure?

1. **No build step for browser**: Data files load as regular scripts, setting globals. `calendar.js` loads as a module and can access those globals.

2. **Separation of generated vs user data**: `calendar-dates.js` can be regenerated without losing user edits in `calendar-status.js`.

3. **Testable with data injection**: Tests import only the pure functions (date parsing, formatting, iteration). Tests create their own test data rather than importing globals—this keeps tests isolated and fast.

4. **`fetch()` doesn't work with `file://`**: Browsers block `fetch()` for local files due to CORS. Using JS files with globals avoids needing a local server.
...
**Implementation:** Located in `scripts/` directory.

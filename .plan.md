...
1. **Generate** — Run `pnpm generate-dates <start> <end>` (e.g. `pnpm generate-dates 2026-02-01 2026-04-04`) to create `calendar-dates.js` with padded date range and Hebrew dates
...
| **3** | Status colors applied from `calendar-statuses.js`                                     | Done   |
| **4** | Shabbos-aware statuses (Saturdays unavailable, Fridays never fully available)         | Done   |
| **5** | Dynamic legend (only shows statuses present in data)                                  | Done   |
| **6** | Hover tooltips (secular date, notes)                                                  | Done   |
| **7** | Hebrew dates: generator computes them + tooltips display them                         | Done   |

---

## Slice 4 — Shabbos-Aware Statuses

### Rules

1. **Saturdays are always unavailable** due to Shabbos — no matter what the user-entered status says. The merge logic overrides any user status to `"unavailable"`.
2. **Saturdays can have notes** — user-entered notes are preserved even though status is forced. (Notes display is not implemented in this slice.)
3. **Fridays are never fully available** due to Erev Shabbos / Leil Shabbos. Valid Friday statuses are: no data, `"partial"`, or `"unavailable"`. If a user enters `"available"` for a Friday, the merge logic downgrades it to `"partial"`.
4. **Fridays can have notes** — preserved as-is. (Notes display is not implemented in this slice.)

### What this slice does NOT include

- Notes rendering (future slice)
- Any UI for editing statuses or notes
...
The `spikes/` directory contains exploratory research notes — time-boxed investigations into how to approach a problem. These are reference material, not production code.

| Spike                                   | Topic                                       |
| --------------------------------------- | ------------------------------------------- |
| [`hebcal-api.md`](spikes/hebcal-api.md) | @hebcal/core API for Hebrew date conversion |

---

## Technical Notes

- **Hebrew dates**: The generator tool computes Hebrew dates using `@hebcal/core` (ESM-only, v6). See [`spikes/hebcal-api.md`](spikes/hebcal-api.md) for API details.
...
```bash
pnpm generate-dates <start> <end>
# e.g. pnpm generate-dates 2026-02-01 2026-04-04
```
